...
fixed_data_arranged %>%
      mutate(first_timestamp = as.character(first_timestamp),
             last_timestamp = as.character(last_timestamp)) %>%
      write_csv('cico_gps_updated.csv')
```
...
points_group %>%
      ...
      write_csv('points_group.csv')
```


GPS RAW DATA INSIDE GEOFENCES GROUPED
```{r}
points_group = read_csv('points_group.csv')
```

```{r}
points_group %>%  
      filter(plateno == 'ACE1174', geofence_name== 'Ritchjoy Marketing') %>%
   arrange(datestamp)
```
