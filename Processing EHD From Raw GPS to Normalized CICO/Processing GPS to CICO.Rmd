...
```{r}
data = read_csv('input_data/ehd_gps_raw.csv')
head(data)
```
Format raw data
...
# Get points inside geofence. Make sure that the gps data are in postgii
...
normalized_cico_to_fo = make_fo(normalized_cico, pattern)
```
