---
title: "nestle data using universal geofence"
author: "Me"
date: "2/3/2021"
...
points_in_geofences <- dbGetQuery(postgrescon, 'SELECT 
	g."site no" AS site_number,
	name AS geofence_name,
	datestamp,
	plateno,
	speed
FROM
	nestle_universal_geofence AS g
JOIN
...
vehicle_location_group_universal = points_in_geofences %>%
      mutate(track_period = floor_date(datestamp, '30 minutes')) %>%
      group_by(plateno, site_number, track_period) %>%
      ...
      arrange(plateno, site_number, track_period) %>%
      select(plateno, site_number, geofence_name, track_period, time_start, time_end, duration, num_points)  %>%
      distinct()

vehicle_location_group_universal
```
Lesser groups 
...
```{r}
morethan5mins = vehicle_location_group_universal %>%
...
morethan5mins = morethan5mins %>%
      group_by(plateno, site_number) %>%
      arrange(plateno, site_number)
...
morethan5mins =  morethan5mins%>%
      ...
      select(plateno, site_number, geofence_name, time_start, actual_time_end) %>%
...
fixed_data = morethan5mins %>%
      group_by(plateno, site_number, actual_time_end) %>%
